...
;; Hashtable operations run it through the async callback path, so this
;; works from inside the async evaluator on a Tokio worker thread. It used
;; to raise "attempt to apply async function in a sync-only context".
(define (loud-hash x)
  (display "")
  x)
