...
use scheme_rs_gc::{
    AllocError, Allocator, BLOCK_SIZE, Global, Heap, LINE_SIZE, META_LINES, Mutator, ObjectModel,
};
...
pub const BUMP_LINE_COUNT: usize = BLOCK_SIZE / LINE_SIZE - META_LINES;
...
pub fn full_block<A: Allocator>(heap: &TestHeap<A>) -> Vec<NonNull<u8>> {
    ...
    let objs: Vec<_> = (0..BUMP_LINE_COUNT)
