(library (match (1))
  (export match match-let match-let* match-lambda match-lambda* match-letrec
          ;; Auxilary keywords:
          _ ___ **1 =.. *.. *** ? $ struct @ object = and or not set! get!)
  (import (rnrs))
  ...
  (define-syntax slot-ref
    (syntax-rules ()
      [(_ record-name rec k)
       ((record-accessor (record-name rtd) k) rec)]))

  (define-syntax slot-set!
    (syntax-rules ()
      [(_ record-name rec k value)
       ((record-mutator (record-name rtd) k) rec value)]))

  (define-syntax is-a?
    (syntax-rules ()
      [(_ rec record-name)
       ((record-predicate (record-name rtd)) rec)]))
